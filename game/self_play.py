...
from game.models import MuZeroNetwork, support_to_scalar
...
class SelfPlay:
    ...
    def recurrent_self_play(self, shared_storage, replay_buffer, test_mode=False):
        while True:
    ...
    def play_game(self, temperature, temperature_threshold, render, opponent, muzero_player):
        ...
        game_history.to_play_history.append(self.game.get_current_player())
        ...
        with torch.no_grad():
            while (
                ...
                # Choose the action
                if opponent == "self" or muzero_player == self.game.get_current_player():
                    ...
                    if render:
                        ...
                        print(
                            "Root value for player {0}: {1:.2f}".format(
                                self.game.get_current_player(), root.value()
                            )
                        )
                else:
                ...
                observation, reward, done = self.game.perform_step(action)

                if render:
                    print(
                        "Played action: {}".format(self.game.convert_action_to_string(action))
                    )
                    self.game.render()
        ...
        if opponent == "human":
            ...
            print(
                "Player {} turn. MuZero suggests {}".format(
                    self.game.get_current_player(),
                    self.game.convert_action_to_string(self.select_action(root, 0)),
                )
            )
            return self.game.get_human_action(), root, tree_depth
        elif opponent == "expert":
            return self.game.expert_action(), None, None
        elif opponent == "random":
            return np.random.choice(self.game.get_legal_actions()), None, None
        else:
            raise ValueError(
